...
export class SaplingParser {
  ...
  public updateTree(filePath: string): Tree {
    ...
    const callback = (node: Tree): void => {
      if (node.filePath === filePath) {
        ...
        this.#traverseTree(matchExpand, newNode as Tree)
  ...
  #traverseTree(callback: Function, node: Tree = this.tree as Tree): void {
    if (!node) {
      return
    }
  ...
  private parser(componentTree: Tree): Tree | void {
    // If import is a node module, do not parse any deeper
    if (!fs.existsSync(componentTree.filePath)) {
    ...
    componentTree.reduxConnect = this.checkForRedux(
      ast.tokens as any[],
      imports
    )
  ...
  private getChildNodes(
    ...
    if (children[astToken.value]) {
      ...
      const { filePath, fileName } = this.getFilePath(children[astToken.value])
      children[astToken.value].filePath = filePath
      children[astToken.value].fileName = fileName
    }
  ...
  private getFilePath(node: Tree) {
    ...
    return { filePath: node.filePath, fileName: node.fileName }
  }
